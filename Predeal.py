...
# 一次遍历，找空白段，找到标点符号位置
def findblank(line, fh, lh, width):
    first = False
    last = False
    lineh = lh - fh + 2
    blanks = []
    ...
    for i in range(len(blanks)):
        if (blanks[i] - font_blank > 5 * font_blank):
            char_blank = blanks[i]
            break
    return char_blank
...
# 找到分号位置，分成两部分
def divideparts(lines, char_blank):
    divide = 0
...
if __name__ == '__main__':
    ...
    for fileName in os.listdir(pic_dir):
        ...
        save_dir = result_dir + save_dir_name + "/"
        ...
        src = cv2.imread(pic_dir + fileName)

        height = src.shape[0]
        width = src.shape[1]
        # print(height,width)
        grayimage = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
        qushuiyin = Qushuiyin(grayimage, height, width)
        # 二次二值化
        ...
        for linenum in range(0, 2):
            ...
            (fh, lh) = cutlines(thresh, newlinebegin, height)
            line = thresh[fh:lh + 2, 0:width]
            char_blank = findblank(line, fh, lh, width)
            (lines, length, lensum) = cutfonts(line, fh, lh, width)
            divide = divideparts(lines, char_blank)
            if divide == 0:
                continue
            (result, pb) = hebing(lines, length, lensum, divide)
